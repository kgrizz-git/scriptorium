...
### Changed
- Book package schema tightened: page images must be at least 1×1; page `file` paths reject traversal/absolutes (including terminal `..`); numeric fields bounded to Rust integer ranges; book `id` constrained to a slug safe as a library directory name; unknown JSON keys rejected at deserialize; `createdAt`/`updatedAt` must be RFC 3339; loaders must clamp or reject out-of-range `lastReadPage` and enforce `pages[i].index == i` / non-zero `byteSize` ([`docs/book-format.md`](docs/book-format.md)).
